...
def _normalize_base_url(base_url: str) -> str:
    """Normalize a base URL for provider API calls."""
    normalized = base_url.rstrip("/").removesuffix("/v1")
    return normalized if normalized.endswith("/") else f"{normalized}/"


def _get_langflow_server_base_url() -> str:
    """Get a default base URL for the current Langflow server."""
    host = os.environ.get("LANGFLOW_HOST", DEFAULT_LANGFLOW_HOST)
    port = os.environ.get("LANGFLOW_PORT", DEFAULT_LANGFLOW_PORT)
    return f"http://{host}:{port}"


def _extract_model_name(model: dict[str, Any]) -> str | None:
    """Extract a model identifier from an OpenWebUI model object."""
...
def _is_openwebui_owned_model(model: dict[str, Any]) -> bool:
    """Return True when model metadata clearly indicates OpenWebUI ownership."""
    owner = str(model.get("owned_by") or model.get("owner") or "").strip().lower()
    source = str(model.get("source") or "").strip().lower()
    model_type = str(model.get("type") or "").strip().lower()
    if model.get("is_custom") is True:
        return True
    if "openwebui" in owner or "open-webui" in owner:
        return True
    if source in {"openwebui", "open-webui", "custom"}:
        return True
    return model_type == "custom"


def _filter_openwebui_model_names(models: list[dict[str, Any]]) -> list[str]:
    """Filter OpenWebUI model list down to custom/OWUI-managed model names."""
    explicit_owned = [
        _extract_model_name(model)
        for model in models
        if _is_openwebui_owned_model(model)
    ]
    explicit_owned = [name for name in explicit_owned if name]
    if explicit_owned:
        return sorted(set(explicit_owned))

    # Fallback: include models that are not explicitly Ollama-owned.
    non_ollama = []
    for model in models:
        owner = str(model.get("owned_by") or model.get("owner") or "").strip().lower()
        if owner == "ollama":
...
async def get_openwebui_models(base_url_value: str, api_key: str | None = None) -> list[str]:
    ...
    async with httpx.AsyncClient() as client:
        for path in OPENWEBUI_MODELS_PATHS:
            endpoint = urljoin(base_url, path)
            with contextlib.suppress(httpx.RequestError, ValueError, TypeError):
                ...
                if models:
                    return _filter_openwebui_model_names(models)
...
async def is_valid_ollama_url(url: str) -> bool:
    """Check if the provided URL is a reachable OpenWebUI/Ollama-compatible endpoint."""
    try:
        transformed_url = transform_localhost_url(url)
        if not transformed_url:
            return False
        base_url = _normalize_base_url(transformed_url)
        async with httpx.AsyncClient() as client:
            for path in OPENWEBUI_MODELS_PATHS:
                with contextlib.suppress(httpx.RequestError):
                    if (await client.get(url=urljoin(base_url, path))).status_code == HTTP_STATUS_OK:
                        return True
            return (await client.get(url=urljoin(base_url, "api/tags"))).status_code == HTTP_STATUS_OK
