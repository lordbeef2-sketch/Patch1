...
def _get_shared_user_ids_from_flow(flow: Flow) -> set[str]:
    tags = flow.tags or []
    return {tag[len(SHARE_TAG_PREFIX) :] for tag in tags if isinstance(tag, str) and tag.startswith(SHARE_TAG_PREFIX)}


def _set_shared_user_ids_on_flow(flow: Flow, shared_user_ids: set[str]) -> None:
    existing_tags = [
        tag for tag in (flow.tags or []) if not (isinstance(tag, str) and tag.startswith(SHARE_TAG_PREFIX))
    ]
    share_tags = [f"{SHARE_TAG_PREFIX}{user_id}" for user_id in sorted(shared_user_ids)]
    flow.tags = [*existing_tags, *share_tags]


def _flow_is_shared_with_user(flow: Flow, user_id: UUID) -> bool:
    return str(user_id) in _get_shared_user_ids_from_flow(flow)


async def _auto_snapshot_flow(
    *,
    ...
    """Create a best-effort flow version snapshot after a successful save."""
    try:
        snapshot_data = copy.deepcopy(flow.data)
        ...
        await logger.awarning("Failed to create flow snapshot for flow %s", flow.id, exc_info=True)
    except Exception:
        await logger.awarning("Unexpected error creating flow snapshot for flow %s", flow.id, exc_info=True)
...
async def read_flows(
    *,
    ...
    try:
        ...
        # Share tags are stored on Flow.tags with the prefix "shared:user:<uuid>".
        # We include shares in Python filtering to avoid backend-specific JSON query differences.
        stmt = select(Flow)
        ...
        if get_all:
            all_candidate_flows = (await session.exec(stmt)).all()
            if auth_settings.AUTO_LOGIN:
                flows = [
                    flow
                    for flow in all_candidate_flows
                    if flow.user_id is None
                    or flow.user_id == current_user.id
                    or _flow_is_shared_with_user(flow, current_user.id)
                ]
            else:
                flows = [
                    flow
                    for flow in all_candidate_flows
                    if flow.user_id == current_user.id or _flow_is_shared_with_user(flow, current_user.id)
                ]
        ...
        # Pagination path keeps owner-only behavior for backward compatibility.
        if auth_settings.AUTO_LOGIN:
            stmt = stmt.where((Flow.user_id == None) | (Flow.user_id == current_user.id))  # noqa: E711
        else:
...
async def _read_flow_accessible(
    session: AsyncSession,
    flow_id: UUID,
    user_id: UUID,
):
    """Read a flow that is either owned by the user or explicitly shared with them."""
    flow = (await session.exec(select(Flow).where(Flow.id == flow_id))).first()
    if flow is None:
        return None
    if flow.user_id == user_id or _flow_is_shared_with_user(flow, user_id):
        return flow
    return None
...
async def read_flow(
    *,
    ...
    """Read a flow."""
    if user_flow := await _read_flow_accessible(session, flow_id, current_user.id):
        # Convert to FlowRead while session is still active to avoid detached instance errors
        return FlowRead.model_validate(user_flow, from_attributes=True)
    raise HTTPException(status_code=404, detail="Flow not found")


@router.get("/{flow_id}/shares", status_code=200)
async def list_flow_shares(
    *,
    ...
    """List users with whom this flow is shared (owner only)."""
    flow = await _read_flow(session, flow_id, current_user.id)
    if not flow:
        raise HTTPException(status_code=404, detail="Flow not found")

    shared_user_ids = _get_shared_user_ids_from_flow(flow)
    if not shared_user_ids:
        return {"flow_id": flow_id, "shared_with": []}

    users = (
        await session.exec(
            select(User).where(col(User.id).in_([UUID(user_id) for user_id in shared_user_ids]))
        )
    ).all()
    return {
        "flow_id": flow_id,
        "shared_with": [{"user_id": str(user.id), "username": user.username} for user in users],
    }


@router.post("/{flow_id}/shares/{target_user_id}", status_code=200)
async def share_flow_with_user(
    *,
    session: DbSession,
    flow_id: UUID,
    target_user_id: UUID,
    ...
    """Share a flow with a specific user (owner only)."""
    flow = await _read_flow(session, flow_id, current_user.id)
    if not flow:
        raise HTTPException(status_code=404, detail="Flow not found")

    if target_user_id == current_user.id:
        raise HTTPException(status_code=400, detail="Cannot share a flow with yourself")

    target_user = (await session.exec(select(User).where(User.id == target_user_id))).first()
    if not target_user:
        raise HTTPException(status_code=404, detail="Target user not found")

    shared_user_ids = _get_shared_user_ids_from_flow(flow)
    shared_user_ids.add(str(target_user_id))
    _set_shared_user_ids_on_flow(flow, shared_user_ids)
...
@router.delete("/{flow_id}/shares/{target_user_id}", status_code=200)
async def unshare_flow_with_user(
    *,
    session: DbSession,
    flow_id: UUID,
    target_user_id: UUID,
    ...
    """Remove explicit share access for a specific user (owner only)."""
    flow = await _read_flow(session, flow_id, current_user.id)
    if not flow:
        raise HTTPException(status_code=404, detail="Flow not found")

    shared_user_ids = _get_shared_user_ids_from_flow(flow)
    shared_user_ids.discard(str(target_user_id))
    _set_shared_user_ids_on_flow(flow, shared_user_ids)
    flow.updated_at = datetime.now(timezone.utc)
    session.add(flow)
    await session.flush()
    ...
    return {
        ...
        "flow_id": str(flow_id),
        "unshared_user_id": str(target_user_id),
    }
