...
async def rollback_to_previous_version(
    ...
    session: DbSession,
    *,
    steps: Annotated[int, Query(ge=1, le=10)] = 1,
    save_draft: Annotated[bool, Query()] = True,
) -> FlowRead:
    """Rollback a flow to a previous saved version.

    `steps=1` rolls back to the immediate previous save, `steps=2` to two saves ago, etc.
    """
    await _get_user_flow(session, flow_id, current_user.id)
    entries = await get_flow_version_list(session, flow_id, current_user.id, limit=steps + 1, offset=0)
    if len(entries) <= steps:
        raise HTTPException(
            status_code=400,
            detail=f"Not enough saved versions to rollback {steps} step(s)",
        )

    target_entry = entries[steps]
    return await activate_version(
        flow_id=flow_id,
        version_id=target_entry.id,
