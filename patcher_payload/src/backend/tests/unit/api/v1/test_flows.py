...
async def test_flow_save_creates_version_snapshots(client: AsyncClient, logged_in_headers):
    create_payload = {
        "name": "snapshot_flow",
        "data": {"nodes": [], "edges": [], "value": "v1"},
    }
    create_response = await client.post("api/v1/flows/", json=create_payload, headers=logged_in_headers)
    assert create_response.status_code == status.HTTP_201_CREATED
    flow_id = create_response.json()["id"]

    update_payload = {
        "data": {"nodes": [], "edges": [], "value": "v2"},
    }
    update_response = await client.patch(f"api/v1/flows/{flow_id}", json=update_payload, headers=logged_in_headers)
    assert update_response.status_code == status.HTTP_200_OK

    versions_response = await client.get(f"api/v1/flows/{flow_id}/versions/", headers=logged_in_headers)
    assert versions_response.status_code == status.HTTP_200_OK
    entries = versions_response.json()["entries"]
    assert len(entries) >= 2


async def test_flow_rollback_restores_previous_saved_data(client: AsyncClient, logged_in_headers):
    create_payload = {
        "name": "rollback_flow",
        "data": {"nodes": [], "edges": [], "value": "before"},
    }
    create_response = await client.post("api/v1/flows/", json=create_payload, headers=logged_in_headers)
    assert create_response.status_code == status.HTTP_201_CREATED
    flow_id = create_response.json()["id"]
    ...
    update_payload = {
        "data": {"nodes": [], "edges": [], "value": "after"},
    }
    update_response = await client.patch(f"api/v1/flows/{flow_id}", json=update_payload, headers=logged_in_headers)
    assert update_response.status_code == status.HTTP_200_OK
    assert update_response.json()["data"]["value"] == "after"

    rollback_response = await client.post(
        f"api/v1/flows/{flow_id}/versions/rollback?steps=1",
        headers=logged_in_headers,
    )
    ...
    assert read_response.json()["data"]["value"] == "before"


async def test_flow_versions_keep_last_10_saves(client: AsyncClient, logged_in_headers, monkeypatch):
    from langflow.services.deps import get_settings_service

    monkeypatch.setattr(get_settings_service().settings, "max_flow_version_entries_per_flow", 10)

    create_payload = {
        "name": "prune_versions_flow",
        "data": {"nodes": [], "edges": [], "value": 0},
    }
    create_response = await client.post("api/v1/flows/", json=create_payload, headers=logged_in_headers)
    assert create_response.status_code == status.HTTP_201_CREATED
    flow_id = create_response.json()["id"]
    ...
    for i in range(1, 15):
        update_payload = {"data": {"nodes": [], "edges": [], "value": i}}
        update_response = await client.patch(f"api/v1/flows/{flow_id}", json=update_payload, headers=logged_in_headers)
        assert update_response.status_code == status.HTTP_200_OK
