...
export default function FlowPage({ view }: { view?: boolean }): JSX.Element {
  ...
  const autoSavingSetting = useFlowsManagerStore((state) => state.autoSaving);
  const setSuccessData = useAlertStore((state) => state.setSuccessData);
  const [isLoading, setIsLoading] = useState(false);
  const initialAutoSavingRef = useRef(autoSavingSetting);

  const normalizeFlowForComparison = (flow?: typeof currentFlow) => {
    if (!flow) {
      return null;
    }

    const nodes = (flow.data?.nodes ?? []).map((node: any) => {
      const {
        selected,
        dragging,
        positionAbsolute,
    ...
    const edges = (flow.data?.edges ?? []).map((edge: any) => {
  ...
  const changesNotSaved =
    customStringify(normalizeFlowForComparison(currentFlow)) !==
    customStringify(normalizeFlowForComparison(currentSavedFlow));
  ...
  const handleSave = () => {
    const pendingAutoSave = useFlowStore.getState().autoSaveFlow as
      | { cancel?: () => void }
      | undefined;
  ...
  useEffect(() => {
    // Keep explicit save/discard/cancel behavior predictable while editing flows.
    if (initialAutoSavingRef.current) {
      setAutoSaving(false);
    }
