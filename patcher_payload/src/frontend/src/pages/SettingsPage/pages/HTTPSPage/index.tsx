    ...
    uploadHTTPSFile(
      ...
      },
      {
        onSuccess: (response) => {
          setFormState((prev) => ({
            ...prev,
            ssl_cert_file: response.ssl_cert_file ?? prev.ssl_cert_file,
            ssl_key_file: response.ssl_key_file ?? prev.ssl_key_file,
          }));
          setP12Password("");
          setSuccessData({
            title:
              "Uploaded PKCS#12 certificate and extracted TLS keypair successfully",
          });
        },
        onError: (error: any) => {
          setErrorData({
      ...
      <div className="flex h-full w-full flex-col gap-6 overflow-x-hidden">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Upload a PKCS#12 bundle to configure TLS. CA-issued certificates
              are supported. Include full certificate chain in the bundle.
              Restart is required after saving HTTPS changes.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div className="md:col-span-2 flex items-center justify-between rounded-md border p-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Requires a valid PKCS#12 bundle that contains both
              ...
              <Form.Field name="p12_file" className="md:col-span-2">
                <Form.Label>PKCS#12 Certificate (.p12/.pfx)</Form.Label>
                <p className="mb-2 text-xs text-muted-foreground">
                  Upload one PKCS#12 bundle and optional password to extract
                  certificate and private key automatically. For CA-issued certs,
                  export with intermediate chain included.
                </p>
                <Input
                  type="password"
                  value={p12Password}
                  onChange={(event) => setP12Password(event.target.value)}
                  placeholder="PKCS#12 password (if required)"
              ...
              <div className="md:col-span-2 rounded-md border p-3 text-sm">
                <p className="font-medium">Extracted TLS Paths</p>
                <p className="text-muted-foreground">
                  Certificate: {formState.ssl_cert_file || "Not uploaded"}
                </p>
                <p className="text-muted-foreground">
                  Private key: {formState.ssl_key_file || "Not uploaded"}
                </p>
              </div>
