...
def get_language_model_options(
    ...
    for provider_data in all_models:
        ...
        for model_data in models:
            ...
            is_live_provider = provider in LIVE_MODEL_PROVIDERS

            # Determine if model should be shown:
            # - For live providers, show all discovered models by default
            # - For static catalogs, if not default and not explicitly enabled, skip it
            # - If in disabled list, skip it
            # - Otherwise, show it
            if not (is_default or is_live_provider) and model_name not in explicitly_enabled_models:
                continue
