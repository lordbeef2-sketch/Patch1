...
async def admin_console_restart_owui(_user=Depends(get_admin_user)):
  stream = get_console_stream()
  await stream.publish("[system] Restart requested by admin; launching replacement process...")

  launch_cmd = shutil.which("owui") or shutil.which("open-webui") or shutil.which("open-webui.exe")
  if launch_cmd:
    cmd = [launch_cmd, "serve"]
  else:
    cmd = [sys.executable, "-c", "import open_webui as owui; owui.serve()"]
